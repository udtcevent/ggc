...
<table class="table table-striped">
    ...
    <tbody>
        <?php
        if ($_GET["onCase"] == 1) {
            $onCase = "ORDER BY id DESC LIMIT 10";
        } else if ($_GET["onCase"] == 2) {
            $onCase = "ORDER BY id DESC LIMIT 50";
        } else if ($_GET["onCase"] == 3) {
            $onCase = "ORDER BY id DESC LIMIT 100";
        } else if ($_GET["onCase"] == 4) {
            $onCase = "ORDER BY id DESC LIMIT 200";
        } else if ($_GET["onCase"] == 5) {
            $onCase = "ORDER BY id DESC LIMIT 300";
        } else {
            $onCase = "ORDER BY id DESC";
        }
        $i = 1;
        $qDataSummon = mysqli_query($conn, "SELECT * FROM _summons $onCase");
