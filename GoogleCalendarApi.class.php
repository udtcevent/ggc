 ...
 */ 
class GoogleCalendarApi { 
    // const API_KEY = 'AIzaSyD_sVEsxJMJPZwjq68PH-8svgj2z2wjB34';
    ...
    public function CreateCalendarEvent($access_token, $calendar_id, $event_data, $all_day, $event_datetime, $event_timezone) { 
        ...
        $event_date_start = !empty($event_datetime['event_date_start'])?$event_datetime['event_date_start']:date("Y-m-d"); 
        $event_date_end = !empty($event_datetime['event_date_end'])?$event_datetime['event_date_end']:date("Y-m-d"); 
        ...
        if($all_day == 1){ 
            $curlPost['start'] = array('date' => $event_date_start); 
            ...
            $dateTime_start = $event_date_start.'T'.$start_time.$timezone_offset; 
    ...
    private function getTimezoneOffset($timezone = 'Asia/Bangkok'){ 
